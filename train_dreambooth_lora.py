...
class DreamBoothDataset(Dataset):
    ...
    def __init__(
        ...
        for path in Path(instance_data_root).iterdir():
            if path.suffix.lower() in valid_extensions:
                ...
                if not txt_path.exists():
                    raise FileNotFoundError(f"Missing prompt file for {path.name}")
                self.instance_images_path.append(path)
                self.prompt_files.append(txt_path)
        ...
        self.default_prompt = instance_prompt  # 保存默认提示词
